...
Available Commands:
  ...
  status     - Show current watcher status (last checked, next check).
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                ...
                if feed.bozo:
                    ...
                    self.logger.warning("Please check your internet connection or try validating the Gengo RSS feed manually.")
                ...
                if not feed.entries:
                    self.logger.info(f"No entries found in the feed. Last checked: {self.last_check_time.strftime('%H:%M:%S')}")
                    self.failure_count = 0
                ...
                if latest_entry.link != self.last_seen_link:
                    ...
                    self.logger.info(f"No new entries. Last checked: {self.last_check_time.strftime('%H:%M:%S')}")
