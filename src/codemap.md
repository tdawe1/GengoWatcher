...
- **Configuration guardrails:** `AppConfig` lazily bootstraps `config.toml`, ensures every section/option exists, preserves typed TOML values (including arrays), and enforces invariants like valid AutoAccept ranges and placeholder detection.
...
- Config modifications propagate via `AppConfig.set`, which writes `config.toml` and is surfaced to CLI commands, interactive prompts, and the web API update endpoints.
