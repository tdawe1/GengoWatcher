...
class CAPTCHAServiceMonitor:
    ...
    def update_performance_metrics(self, service_name: str, captcha_manager):
        ...
        try:
            ...
            if service_name in stats.get("service_stats", {}):
                ...
                avg_response_time = (
                    sum(solve_times) / max(1, len(solve_times)) if solve_times else 0.0
                )
            ...
            self.logger.error(
                f"Error updating performance metrics for {service_name}: {e}"
            )
    ...
    def start_monitoring(self, captcha_manager, interval: int = 300):
        ...
        self._monitoring_thread = threading.Thread(
            target=monitor_loop, name="CAPTCHAServiceMonitor", daemon=True
        )
    ...
    def log_health_status(self):
        ...
        for service_name, health in health_status.items():
            ...
            self.logger.info(
                f"    Last Checked: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(health.last_checked))}"
            )
            if health.details:
    ...
    def log_performance_metrics(self):
        ...
        for service_name, perf in metrics.items():
            ...
            self.logger.info(
                f"    Average Response Time: {perf.avg_response_time:.2f}s"
            )
            ...
            self.logger.info(
                f"    Last Updated: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(perf.last_updated))}"
            )
