...
class BaseCaptchaSolver(ABC):
    ...
    def _make_request(
        self, method: str, url: str, timeout: tuple = None, **kwargs
    ) -> Dict[str, Any]:
...
class TwoCaptchaSolver(BaseCaptchaSolver):
    ...
    def get_balance(self) -> float:
        ...
        with self._balance_lock:
            ...
            if (
                ...
                and (current_time - self._last_balance_check) < self._balance_cache_ttl
            ):
            ...
            try:
                ...
                if response.get("status") == 1:
                    ...
                    raise CaptchaSolverAPIError(
                        f"Failed to get balance: {response.get('request')}"
                    )
    ...
    def solve_recaptcha_v2(
        ...
        if balance <= 0:
            self.logger.warning(
                "Insufficient balance for CAPTCHA solving",
                extra={
        ...
        if response.get("status") != 1:
            ...
            self.logger.error(
                "Failed to submit reCAPTCHA v2",
                extra={"service": self.get_service_name(), "error": error_msg},
            )
        ...
        try:
            ...
            self.logger.info(
                "Successfully solved reCAPTCHA v2",
                extra={
                    "service": self.get_service_name(),
                    "task_id": task_id,
            ...
            return CaptchaSolution(
                captcha_id=task_id, solution=solution, solved_at=time.time(), cost=cost
            )
        except Exception as e:
            poll_time = time.time() - poll_start_time
            total_time = time.time() - start_time
    ...
    def solve_recaptcha_v3(
        ...
        try:
            ...
            self._log_event(
                "info",
                "Successfully solved reCAPTCHA v3",
                service=self.get_service_name(),
                task_id=task_id,
                solution_length=len(solution) if solution else 0,
                cost=cost,
    ...
    def _poll_for_result(
        self, task_id: str, polling_interval: float = 5, max_wait_time: int = 300
    ) -> Dict[str, Any]:
        """
        ...
        while time.time() - start_time < max_wait_time:
            ...
            if status == 1:
                ...
                if request == "CAPCHA_NOT_READY":
                    ...
                    self.logger.warning(
                        f"Unexpected 2Captcha response: status={status}, request={request}"
                    )
            else:
                # Invalid status
            ...
            current_polling_interval = min(
                current_polling_interval * 1.5, max_polling_interval
            )
...
class AntiCaptchaSolver(BaseCaptchaSolver):
        ...
        with self._balance_lock:
            ...
            try:
                ...
                if response.get("errorId") == 0:
                    ...
                    raise CaptchaSolverAPIError(
                        f"Failed to get balance: {response.get('errorDescription')}"
                    )
            except Exception as e:
    ...
    def solve_recaptcha_v2(
        ...
        response = self._make_request(
            "POST", f"{self.BASE_URL}/createTask", timeout=(10, 30), json=task_data
        )
        ...
        self._log_event(
            "info",
            "Submitted reCAPTCHA v2 task to Anti-Captcha",
            service=self.get_service_name(),
            task_id=task_id,
            submit_time=round(submit_time, 3),
        )
    ...
    def solve_recaptcha_v3(
        ...
        if response.get("errorId") != 0:
            raise CaptchaSolverAPIError(
                f"Failed to submit CAPTCHA: {response.get('errorDescription')}"
            )
        ...
        return CaptchaSolution(
            captcha_id=str(task_id), solution=solution, solved_at=time.time(), cost=cost
        )
            ...
            if error_id == 0:
                ...
                if status == "ready":
                    ...
                    raise CaptchaSolverError(
                        f"Anti-Captcha returned unexpected status: {status}"
                    )
            else:
                # Error occurred
