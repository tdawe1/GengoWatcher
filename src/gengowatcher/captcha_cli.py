...
def setup_captcha_solver(watcher):
    ...
    try:
        ...
        watcher.captcha_solver = CaptchaSolverManager(
            watcher.config.config, watcher.logger
        )
...
def show_captcha_stats(watcher):
    ...
    service_name = (
        watcher.captcha_solver.solver.get_service_name()
        if watcher.captcha_solver.solver
        else "Unknown"
    )
    balance = watcher.captcha_solver.get_balance()
    ...
    if stats["error_stats"]:
        ...
        for error_type, count in sorted(
            stats["error_stats"].items(), key=lambda x: x[1], reverse=True
        ):
...
def reset_captcha_config(watcher):
    ...
    confirm = (
        input(f"Are you sure you want to reset {service} configuration? (y/N): ")
        .strip()
        .lower()
    )
