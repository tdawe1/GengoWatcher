...
class CaptchaSolverManager:
        ...
        try:
            ...
            if service_type == "local":
                try:
                    ...
                    local_solver_manager = LocalCaptchaSolverManager(
                    ...
                    if local_solver_manager.is_supported():
                        ...
                        self.logger.warning(
                            "Local CAPTCHA solver not supported on this system"
                        )
                except ImportError:
            ...
            self.solver = CaptchaServicePluginFactory.create_plugin(
                service_type, self.config, self.logger
            )

            if self.solver:
                self.logger.info(
                    f"Initialized {self.solver.get_service_name()} solver via plugin system"
                )
            else:
                self.logger.warning(
                    f"Failed to initialize CAPTCHA solver for service: {service_type}"
                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            stats["success_rate"] = (
                stats["solved_count"] / max(1, total_attempts) * 100
                if total_attempts > 0
                else 0.0
            )
            ...
            if stats["solve_times"]:
                stats["avg_solve_time"] = sum(stats["solve_times"]) / len(
                    stats["solve_times"]
                )
            ...
            for service_name, service_stats in stats["service_stats"].items():
                ...
                service_stats["success_rate"] = (
                    service_stats["solved"] / max(1, service_total) * 100
                    if service_total > 0
                    else 0.0
                )
                if service_stats["solve_times"]:
                    service_stats["avg_solve_time"] = sum(
                        service_stats["solve_times"]
                    ) / len(service_stats["solve_times"])
            ...
            for captcha_type, type_stats in stats["captcha_type_stats"].items():
                ...
                type_stats["success_rate"] = (
                    type_stats["solved"] / max(1, type_total) * 100
                    if type_total > 0
                    else 0.0
                )
    ...
    def solve_image_captcha(self, image_path: str) -> Optional[CaptchaSolution]:
        ...
        if hasattr(self, "_local_solver_manager") and self._local_solver_manager:
            ...
            self.logger.warning(
                "Local CAPTCHA solver not available for image CAPTCHA solving"
            )
            return None
    ...
    def solve_recaptcha_v2(
        ...
        for attempt in range(max_retries):
            try:
                self.logger.info(
                    f"Solving reCAPTCHA v2 (attempt {attempt + 1}/{max_retries})"
                )
                ...
                with self._lock:
                    ...
                    self._stats["service_stats"][service_name]["solve_times"].append(
                        solve_time
                    )
                    # Keep only last 100 solve times
                    if (
                        len(self._stats["service_stats"][service_name]["solve_times"])
                        > 100
                    ):
                ...
                self.logger.warning(
                    f"CAPTCHA solving timed out (attempt {attempt + 1}): {e}"
                )
                with self._lock:
                    self._stats["failed_count"] += 1
                    self._stats["captcha_type_stats"]["recaptcha_v2"]["failed"] += 1
                ...
                self.logger.warning(
                    f"CAPTCHA solving failed (attempt {attempt + 1}): {e}"
                )
                with self._lock:
                    self._stats["failed_count"] += 1
                    self._stats["captcha_type_stats"]["recaptcha_v2"]["failed"] += 1
    ...
    def solve_recaptcha_v3(
        ...
        for attempt in range(max_retries):
            try:
                self.logger.info(
                    f"Solving reCAPTCHA v3 (attempt {attempt + 1}/{max_retries})"
                )
                solution = self.solver.solve_recaptcha_v3(
                    site_key, page_url, action, **kwargs
                )
                ...
                with self._lock:
                    ...
                    self._stats["captcha_type_stats"]["recaptcha_v3"][
                        "total_cost"
                    ] += cost
                    # Update service stats
                    service_name = self.solver.get_service_name()
    ...
    def solve_hcaptcha(
        ...
        for attempt in range(max_retries):
            try:
                self.logger.info(
                    f"Solving hCaptcha (attempt {attempt + 1}/{max_retries})"
                )
    ...
    def handle_job_rejection(self, job_data: Dict[str, Any]) -> bool:
        ...
        self.logger.info(
            f"Handling job rejection that requires CAPTCHA solving: {job_data.get('id')}"
        )
    ...
    def _submit_captcha_solution(
        self, job_data: Dict[str, Any], solution: CaptchaSolution
    ):
    ...
    def log_stats(self):
        ...
        if stats["last_solved_at"]:
            self.logger.info(
                f"  Last solved: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(stats['last_solved_at']))}"
            )
        ...
        for captcha_type, type_stats in stats["captcha_type_stats"].items():
            self.logger.info(
                f"    {captcha_type}: Solved={type_stats['solved']}, Failed={type_stats['failed']}, "
        ...
        if stats["service_stats"]:
            ...
            for service_name, service_stats in stats["service_stats"].items():
                self.logger.info(
                    f"    {service_name}: Solved={service_stats['solved']}, Failed={service_stats['failed']}, "
                    f"Success Rate={service_stats['success_rate']:.1f}%, Cost=${service_stats['total_cost']:.4f}"
                )
                if service_stats["solve_times"]:
                    self.logger.info(
                        f"      Solve Times: Avg={service_stats['avg_solve_time']:.2f}s, "
        ...
        if stats["solve_times"]:
            self.logger.info(
                f"  Overall Performance: Avg={stats['avg_solve_time']:.2f}s, "
