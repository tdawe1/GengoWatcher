...
class CaptchaServicePlugin(BaseCaptchaSolver):
    ...
    def __init__(self, config: Dict[str, Any], logger: logging.Logger = None):
        ...
        if not api_key:
            raise CaptchaSolverError(f"API key for {self.service_identifier} not found in secure storage")
    ...
    def solve_captcha(self, task: CaptchaTask) -> CaptchaSolution:
        ...
        if task.captcha_type == CaptchaType.RECAPTCHA_V2:
            ...
            return self.solve_recaptcha_v3(task.site_key, task.page_url, task.action or "verify")
