...
class TensorFlowCaptchaSolver(BaseLocalCaptchaSolver):
    ...
    def __init__(self, config: Dict[str, Any], logger: logging.Logger = None):
        ...
        self.model = None
        self.model_path = config.get("LocalCaptcha", {}).get(
    ...
    def solve_image_captcha(self, image_path: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.debug(
                "Processed image shape: %s",
                getattr(processed_image, "shape", "unknown"),
            )
...
def create_tensorflow_solver(
    config: Dict[str, Any], logger: logging.Logger
) -> Optional[TensorFlowCaptchaSolver]:
