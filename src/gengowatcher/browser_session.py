...
def build_browser_aligned_websocket_headers(
    *,
    ...
    """Build WebSocket headers matching real Firefox browser fingerprint.

    Header order matters for TLS/JA3 fingerprinting - matches Firefox's order:
    Host, User-Agent, Accept, Accept-Language, Accept-Encoding,
    Sec-WebSocket-Version, Origin, Sec-WebSocket-Extensions,
    Sec-WebSocket-Key (generated by websockets lib), Sec-GPC,
    Connection, Cookie

    If cookie_header provided, use it directly (full cookie jar). Otherwise
    fall back to just session token.
    """
    headers = {
        "Host": "live-dashboard.gengo.com",
        "User-Agent": user_agent or "Mozilla/5.0 (X11; Linux x86_64; rv:152.0) Gecko/20100101 Firefox/152.0",
        "Accept": "*/*",
    ...
    if cookie_header:
        ...
        headers["Cookie"] = f"myG_myGSession_={session_token}; myG_rdsessID={session_token}"
    return headers
