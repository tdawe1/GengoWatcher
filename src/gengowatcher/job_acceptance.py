...
class JobAcceptanceEngine:
    ...
    async def initialize_session(self):
        ...
        if self.session is None or self.session.closed:
            ...
            # Use log level DEBUG if it is enabled, otherwise INFO
            if self.logger.isEnabledFor(logging.DEBUG):
                self.logger.debug("HTTP session initialized for job acceptance")
            else:
                self.logger.info("HTTP session initialized for job acceptance")
    ...
    def is_job_eligible(self, job_data: Dict[str, Any]) -> bool:
        ...
        job_sources = self.config.get("AutoAccept", "job_sources")
        if job_sources is None:
            # Fallback to default if config returns None (though config should handle defaults)
            job_sources = "rss,websocket"

        allowed_sources = {
            s.strip() for s in job_sources.split(",")
        }
    ...
    async def _attempt_job_acceptance(self, job_data: Dict[str, Any]) -> AcceptResult:
        ...
        result = await self._wait_for_winner(
            tasks, cancel_event, attempt_timeout, template_timings
        )
    ...
    async def _attempt_selenium_accept(
        ...
        probe_ms = self.config.getint(
            "AutoAccept", "accept_click_probe_ms", fallback=75
        )
                                ...
                                if (
                                    ...
                                    if (
                                        self.config.getboolean(
                                            "Captcha",
                                            "enable_browser_automation_fallback",
                                            True,
                                        )
                                        ...
                                        try:
                                            ...
                                            if ok:
                                                self.logger.info(
                                                    f"Browser fallback submission appears successful for job {job_id}"
                                                )
                                                mark_timing("redirect_ms")
                                                return True
                                        except Exception as be:
                                            self.logger.warning(
                                                f"Browser fallback failed: {be}"
                                            )
                                    return False
                            else:
                                self.logger.error(
            ...
            if recaptcha_v3_scripts:
                ...
                if not site_key:
                    ...
                    if self.config.getboolean(
                        ...
                        if self.config.getboolean(
                            "Captcha", "enable_browser_automation_fallback", False
                        ):
                            self.logger.info(
                                f"Attempting browser automation fallback for reCAPTCHA v3 for job {job_id}"
                            )
