...
class SimpleLocalCaptchaSolver(BaseLocalCaptchaSolver):
    ...
    def solve_image_captcha(self, image_path: str) -> Optional[str]:
        ...
        self.logger.warning(
            f"Simple solver cannot actually solve CAPTCHA: {image_path}"
        )
        self.logger.info(
            "In a real implementation, this would use ML models to solve the CAPTCHA"
        )
...
class LocalCaptchaSolverManager:
    ...
    def _load_solvers(self):
        ...
        with self._lock:
            ...
            # Set active solver
            preferred_solver = self.config.get("LocalCaptcha", {}).get(
            ...
            if preferred_solver in self._solvers:
                ...
                self.logger.info(
                    f"Using local CAPTCHA solver: {self._active_solver.get_service_name()}"
                )
